...
func BuyItem(purchaseDetails *rblx.PurchasePost, snipeAccountSession *rblx.RBLXSession) {
	...
	if purchaseError != nil {
		...
		case rblx.TooManyRequests:
			fmt.Printf("[Reaper] Got too many requests error while purchasing asset %d, cookie might have expired. \n", purchaseDetails.AssetID)
		case rblx.AuthorizationDenied:
			fmt.Printf("[Reaper] Authorization failed while purchasing asset %d, cookie might have expired. \n", purchaseDetails.AssetID)
		case rblx.TokenValidation:
			fmt.Printf("[Reaper] Token validation failed while purchasing asset %d \n", purchaseDetails.AssetID)
		case rblx.UnknownError:
			fmt.Printf("[Reaper] An unknown error occurred while purchasing asset %d. Error: %s [REASON] %s \n", purchaseDetails.AssetID, purchaseError.Err.Error(), purchaseResponse)
		}
		return
	}

	if purchaseResponse == nil {
		fmt.Printf("[Reaper] An unknown error occured while purchasing asset %d. Error: idk tbh \n", purchaseDetails.AssetID)
	}

	if purchaseResponse.Purchased == true {

		//Picks random good Quote
		...
		//checks if autosell is on
		if globals.Config.AutoSell == true {

			// List item for sale
			sellItemResponse, err := snipeAccountSession.SellItem(purchaseDetails.AssetID, purchaseDetails.UserAssetID, resellerDetails.Data[1].Price)

			if err != nil {
				fmt.Println("[Reaper] Failed to list item for sale due to unknown error")
				fmt.Println(sellItemResponse)
				return
			}

			//checks if response isnt nil (fucks up and crashes at that)
			if sellItemResponse == nil {
				fmt.Println("[Reaper] Failed to list item for sale due to unknown error")
				fmt.Println(sellItemResponse)
				return
			}

			if sellItemResponse.Sold == false {
				// Create resell embed
				rand.Seed(time.Now().UnixNano())
				...
				fmt.Printf("[REAPER] Resold (Glitched) %s for sniped price %d! [REASON] %s \n", globals.CachedAssetNames[purchaseDetails.AssetID], purchaseDetails.ExpectedPrice, purchaseResponse.Reason)
			}

		}
	} else {

		// Picks random bad quote
		rand.Seed(time.Now().UnixNano())
		badQuote := globals.NegativeQuotes[rand.Intn(len(globals.NegativeQuotes))]
		...
		//Sends webhook to custom webhook
		embed.SetFooter(fmt.Sprintf("%s | ~%s", badQuote, globals.Config.Alias), fmt.Sprintf("%s", globals.Config.ProfileAvatar))
		webhookEmbed, _ := json.Marshal(embed)
		...
		//Sends webhook to reaper scythes webhook
		...
		fmt.Printf("[REAPER] Failed to scythe %s for %d! [REASON] %s \n", globals.CachedAssetNames[purchaseDetails.AssetID], purchaseDetails.ExpectedPrice, purchaseResponse.Reason)
	}
}
